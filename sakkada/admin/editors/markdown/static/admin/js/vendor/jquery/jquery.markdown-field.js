/**
 *  Markdown interactive text field previewer powered by marked library
  ...
  $.fn.extend({
    markdownPreview: function(options) {
      ...
      this.each(function() {
        ...
        // allow only for textareas and text inputs and only once
        if (!field.is('textarea, input[type=text]') ||
            field.data('editor-markdown-enabled')) return;
        ...
        // initial resizing and field lock
        field.trigger('mouseup');
        field.data('editor-markdown-enabled', true);
